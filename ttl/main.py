...
class RMQSender():
    def __init__(self, exchange="messages", host='localhost', user='guest', password='guest', virtual_host="/", queue=""):
    ...
    def send_msg(self, msg, ttl=0):
        ...
        self.channel.basic_publish(
            exchange=self.exchange, routing_key=self.queue, body=msg, properties=properties)


if __name__ == '__main__':
    rmq_connection = RMQSender(
        host=HOST, user=USER, password=PASSWORD, virtual_host=VHOST, queue="ttl_100")
    rmq_connection.send_msg(msg, ttl=TTL)
